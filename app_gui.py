...
class AppGUI:
    ...
    def run(self):
        """Starts the Tkinter main loop and schedules the status update."""
    ...
    # --- NEW: Method to add the visual separator ---
    def add_reset_separator(self):
        """Adds a separator line to the feed to indicate a stream reset."""
        def _task():
            # Apply the same smart scrolling logic as a regular response
            scroll_pos = self.feed_text.yview()
            is_at_top = scroll_pos[0] == 0.0
            top_index = self.feed_text.index("@0,0")

            self.feed_text.configure(state=tk.NORMAL)
            # A unicode character for a solid line works well here
            separator_text = "─" * 80 
            self.feed_text.insert('1.0', f"\n{separator_text}\n\n", "separator")
            if not is_at_top:
                self.feed_text.see(top_index)
            self.feed_text.configure(state=tk.DISABLED)
            # Define the style for our new separator
            self.feed_text.tag_config("separator", foreground="#03A9F4", justify='center') # Light Blue
        self.root.after(0, _task)
